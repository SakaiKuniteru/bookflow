const service = require('./kho.service.js');
...
class KhoController {
    xuLy(hanhDong, status = 200) {
        return async (req, res, next) => {
            try {
                const bang = req.loaiBang ?? 'kho';
                const id = req.params.banGhiId ?? req.params.khoId;
                const chaId = req.loaiBang ? req.params.khoId : null;
                let data;
