...
class PhanQuyenRepository {
    async layQuyenHieuLuc(taiKhoanId, donViId, maQuyen = null, client) {
    ...
    async chiNhanhHoatDong(donViId, chiNhanhId, client) {
        const { rows } = await query(
    ...
    async danhSachVaiTro(donViId, client) {
        const { rows } = await query(
    ...
    async layVaiTro(donViId, vaiTroId, client, khoa = false) {
    ...
    async taoVaiTro(donViId, actorId, duLieu, client) {
        const { rows } = await query(
    ...
    async suaVaiTro(donViId, vaiTroId, actorId, duLieu, client) {
        const { rows } = await query(
    ...
    async voHieuVaiTro(donViId, vaiTroId, actorId, client) {
        const { rows } = await query(
    ...
    async quyenCuaVaiTro(donViId, vaiTroId, client) {
        const { rows } = await query(
    ...
    async quyenTheoMa(cacMaQuyen, client) {
        const { rows } = await query(
    ...
    async xoaQuyenVaiTro(donViId, vaiTroId, client) {
        await query('DELETE FROM vai_tro_quyen WHERE don_vi_id = $1 AND vai_tro_id = $2', [donViId, vaiTroId], client);
    }

    async ganQuyenVaiTro(donViId, vaiTroId, quyenId, phamVi, actorId, client) {
        await query(
    ...
    async thanhVienTheoTaiKhoan(donViId, taiKhoanId, client) {
        const { rows } = await query(
    ...
    async thanhVienCoChiNhanh(donViId, thanhVienId, chiNhanhId, client) {
        const { rows } = await query(
            `SELECT EXISTS (
    ...
    async vaiTroCoQuyenDonVi(donViId, vaiTroId, client) {
        const { rows } = await query(
    ...
    async vaiTroDaGan(donViId, thanhVienId, vaiTroId, chiNhanhId, client) {
        const { rows } = await query(
    ...
    async ganVaiTro(donViId, thanhVienId, vaiTroId, chiNhanhId, actorId, nguoiCapId, client) {
        const { rows } = await query(
    ...
    async vaiTroCuaThanhVien(donViId, thanhVienId, client) {
        const { rows } = await query(
    ...
    async thuHoiVaiTro(donViId, thanhVienId, ganVaiTroId, actorId, client) {
        const { rows } = await query(
...
module.exports = new PhanQuyenRepository();