...
import { capNhatDonViHopLe, donViMoiHopLe } from './don-vi.validation.js';
import * as repo from './don-vi.repository.js';

function loi(message, status = 403, code = 'FORBIDDEN') {
    return new AppError({ code, message, status });
}

export async function taoDonVi(auth, body, requestId) {
    ...
    return trongGiaoDich(async client => {
        ...
        await repo.ghiNhatKyDonVi({
            donViId: donVi.id, actorId: taiKhoan.id,
            hanhDong: 'tenant.create', requestId
        }, client);
        return { ...donVi, don_vi_dang_chon_id: donVi.id };
    });
}

export async function danhSachDonVi(auth) {
...
export async function donViHienTai(auth) {
...
export async function capNhatDonVi(auth, body, requestId) {
    ...
    return trongGiaoDich(async client => {
        ...
        if (!await kiemTraQuyen(auth, 'units.manage', {}, client)) throw loi('Không có quyền cập nhật đơn vị');
        const donVi = await repo.capNhatDonViDb(auth.donViId, duLieu, client);
        if (!donVi) throw loi('Không tìm thấy đơn vị', 404, 'NOT_FOUND');
        await repo.ghiNhatKyDonVi({
            donViId: donVi.id, actorId: auth.taiKhoanId,
            hanhDong: 'tenant.update', requestId
        }, client);
        return donVi;
    });
}