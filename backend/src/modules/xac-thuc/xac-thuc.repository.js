...
export async function timTaiKhoan(dinhDanh, client) {
    const { rows } = await query(
        'SELECT * FROM tai_khoan WHERE lower(email) = $1 OR lower(ten_dang_nhap) = $1 LIMIT 1',
        [dinhDanh], client
    );
    return rows[0] ?? null;
}

export async function timTaiKhoanTheoEmail(email, client) {
    const { rows } = await query('SELECT * FROM tai_khoan WHERE lower(email) = $1 LIMIT 1', [email], client);
    return rows[0] ?? null;
}

export async function khoaTaiKhoan(id, client) {
    const { rows } = await query('SELECT * FROM tai_khoan WHERE id = $1 FOR UPDATE', [id], client);
    return rows[0] ?? null;
}

export async function taoOtp(taiKhoanId, email, mucDich, maBam, soLanGui, client) {
    const { rows } = await query(
...
export async function otpGanNhat(taiKhoanId, mucDich, client) {
    const { rows } = await query(
...
export async function soOtpTrongNgay(taiKhoanId, mucDich, client) {
    const { rows } = await query(
...
export async function huyOtpCu(taiKhoanId, mucDich, client) {
    await query(
        'UPDATE ma_xac_minh SET ngay_su_dung = now() WHERE tai_khoan_id = $1 AND muc_dich = $2 AND ngay_su_dung IS NULL',
        [taiKhoanId, mucDich], client
    );
}

export async function tangLanThuOtp(id, client) {
    await query('UPDATE ma_xac_minh SET so_lan_thu = so_lan_thu + 1 WHERE id = $1 AND so_lan_thu < 5', [id], client);
}

export async function suDungOtp(id, client) {
    await query('UPDATE ma_xac_minh SET ngay_su_dung = now() WHERE id = $1 AND ngay_su_dung IS NULL', [id], client);
}

export async function xacMinhEmail(taiKhoanId, client) {
    const { rows } = await query(
...
export async function taoPhienDb(taiKhoanId, maPhienBam, csrfBam, client, donViId = null, chiNhanhId = null) {
    const { rows } = await query(
        `INSERT INTO phien_dang_nhap (tai_khoan_id, ma_phien_bam, csrf_token_bam, ngay_het_han,
            phien_ban_xac_thuc, nen_tang, don_vi_dang_chon_id, chi_nhanh_dang_chon_id)
         SELECT id, $2, $3, now() + interval '7 days', phien_ban_xac_thuc, 'WEB', $4, $5
         FROM tai_khoan
         WHERE id = $1 AND trang_thai = 'DANG_DUNG' AND email_da_xac_minh = TRUE
           AND bat_buoc_doi_mat_khau = FALSE
         RETURNING id, ngay_het_han`,
        [taiKhoanId, maPhienBam, csrfBam, donViId, chiNhanhId], client
    );
    return rows[0] ?? null;
}

export async function layPhienTheoToken(maPhienBam, client) {
    const { rows } = await query(
        `SELECT p.id, p.tai_khoan_id, p.don_vi_dang_chon_id, p.chi_nhanh_dang_chon_id,
            p.csrf_token_bam, t.email, t.ho_ten
         FROM phien_dang_nhap p JOIN tai_khoan t ON t.id = p.tai_khoan_id
         WHERE p.ma_phien_bam = $1 AND p.ngay_thu_hoi IS NULL AND p.ngay_het_han > now()
           AND p.phien_ban_xac_thuc = t.phien_ban_xac_thuc AND t.trang_thai = 'DANG_DUNG'
           AND t.email_da_xac_minh = TRUE AND t.bat_buoc_doi_mat_khau = FALSE
...
export async function thuHoiPhien(id, lyDo, client) {
...
export async function thuHoiTatCaPhien(id, lyDo, client) {
    await query(
        `UPDATE phien_dang_nhap SET ngay_thu_hoi = now(), ly_do_thu_hoi = $2
...
export async function capNhatMatKhau(id, matKhauBam, client, kichHoat = false) {
    const { rows } = await query(
        `UPDATE tai_khoan SET mat_khau_bam = $2, phien_ban_xac_thuc = phien_ban_xac_thuc + 1,
         ...
         WHERE id = $1 RETURNING id, email, ho_ten`,
        [id, matKhauBam, kichHoat], client
    );
    return rows[0] ?? null;
}

export async function tangDangNhapSai(id, client) {
    await query(
...
export async function dangNhapThanhCong(id, client) {
    await query(
        'UPDATE tai_khoan SET so_lan_dang_nhap_sai = 0, khoa_den = NULL, ngay_dang_nhap_cuoi = now() WHERE id = $1',
        [id], client
    );
}

export async function ghiNhatKyDangNhap(id, dinhDanh, ketQua, client) {
    await query(
        'INSERT INTO nhat_ky_dang_nhap (tai_khoan_id, email_da_nhap, ket_qua) VALUES ($1, $2, $3)',
        [id, dinhDanh.includes('@') ? dinhDanh : null, ketQua], client
    );
}

export async function capNhatThanhVienKichHoat(taiKhoanId, donViId, client) {
    await query(
...
export async function chonDonViPhien(phienId, taiKhoanId, donViId, client) {
    const { rows } = await query(
...
export async function coQuyenQuanLyNhanVien(taiKhoanId, donViId, client) {
    const { rows } = await query(
        `SELECT EXISTS (
...
export async function taoTaiKhoanNhanVien({ email, hoTen, tenDangNhap, matKhauBam, donViId }, client) {
    const { rows } = await query(
...
export async function taoThanhVien({ donViId, taiKhoanId, maNhanVien, chucDanh, email, soDienThoai, nguoiTaoId }, client) {
    const { rows } = await query(
...
export async function ganVaiTroNhanVien(donViId, thanhVienId, client) {
    const { rows } = await query(
...
export async function layNhanVienChoKichHoat(taiKhoanId, donViId, client) {
    const { rows } = await query(
...
export async function datLaiMatKhauTam(taiKhoanId, matKhauBam, client) {
    await query(
        `UPDATE tai_khoan SET mat_khau_bam = $2, phien_ban_xac_thuc = phien_ban_xac_thuc + 1,
...
export async function ghiAuditNhanVien({ donViId, actorId, targetId, requestId, hanhDong }, client) {
    await query(
