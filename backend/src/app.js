...
export function taoUngDung({ kiemTraSanSang = null } = {}) {
    ...
    app.get('/ready', async (req, res, next) => {
        try {
            const ketQua = kiemTraSanSang
                ? await kiemTraSanSang()
                : { database: false, redis: false, storage: false };
            if (!Object.values(ketQua).every((giaTri) => giaTri === true)) {
                throw new AppError({
                    code: 'SERVICE_UNAVAILABLE',
                    message: 'Dịch vụ chưa sẵn dàng',
                    status: 503
                });
            }
            res.status(200).json({
